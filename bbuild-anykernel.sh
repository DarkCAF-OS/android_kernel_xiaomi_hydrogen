...
TOOLCHAIN="/home/legacy/n/prebuilts/gcc/linux-x86/aarch64/aarch64-linux-android-4.9/bin/aarch64-linux-android-"
ARCHITECTURE=arm64
...
NEBULA_FILENAME="${KERNEL_NAME,,}-$NEBULA_VERSION"
...
step0_copy_code()
{
	...
	sed "s/\`echo \$LINUX_COMPILE_BY | \$UTS_TRUNCATE\`/$KERNEL_NAME-$NEBULA_VERSION/g" -i $BUILD_PATH/scripts/mkcompile_h
	sed "s/\`echo \$LINUX_COMPILE_HOST | \$UTS_TRUNCATE\`/Team-Nebula/g" -i $BUILD_PATH/scripts/mkcompile_h
}
...
step3_compile()
{
	...
	if [ "y" == "$COMPILE_DTB" ]; then
		...
		chmod 777 tools_nebula/$DTBTOOL
		tools_nebula/$DTBTOOL $DTBTOOL_CMD -o $BUILD_PATH/$OUTPUT_FOLDER/arch/$ARCHITECTURE/boot/dt.img -s 2048 -p $BUILD_PATH/$OUTPUT_FOLDER/scripts/dtc/ $BUILD_PATH/$OUTPUT_FOLDER/arch/$ARCHITECTURE/boot/
	fi
...
step4_prepare_anykernel()
{
	...
	cd $REPACK_PATH
	cp -R $BUILD_PATH/anykernel_nebula/* .
...

	{
		...
		if [ "$(ls -A $BUILD_PATH/modules_nebula)" ]; then
			cp $BUILD_PATH/modules_nebula/* $MODULES_PATH
			cd $MODULES_PATH
	...
	cd $REPACK_PATH
	KERNELNAME="Flashing $KERNEL_NAME $NEBULA_VERSION"
...
step5_create_anykernel_zip()
{
	...
	# create zip file
	cd $REPACK_PATH
	zip -r9 $NEBULA_FILENAME.recovery.zip * -x $NEBULA_FILENAME.recovery.zip

	# sign recovery zip if there are keys available
	if [ -f "$BUILD_PATH/tools_nebula/testkey.x509.pem" ]; then
		echo -e ">>> signing recovery zip\n"
		java -jar $BUILD_PATH/tools_nebula/signapk.jar -w $BUILD_PATH/tools_nebula/testkey.x509.pem $BUILD_PATH/tools_nebula/testkey.pk8 $NEBULA_FILENAME.recovery.zip $NEBULA_FILENAME.recovery.zip_signed
		cp $NEBULA_FILENAME.recovery.zip_signed $NEBULA_FILENAME.recovery.zip
		rm $NEBULA_FILENAME.recovery.zip_signed
	fi

	md5sum $NEBULA_FILENAME.recovery.zip > $NEBULA_FILENAME.recovery.zip.md5

	# Creating additional files for load&flash
	echo -e ">>> create load&flash files\n"

	cp $NEBULA_FILENAME.recovery.zip cm-kernel.zip
...
step8_transfer_kernel()
{
	...
	if [ ! -z "$SMB_SHARE_KERNEL" ]; then
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "mkdir $SMB_FOLDER_KERNEL\\$NEBULA_VERSION"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$NEBULA_FILENAME.recovery.zip $SMB_FOLDER_KERNEL\\$NEBULA_VERSION\\$NEBULA_FILENAME.recovery.zip"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$NEBULA_FILENAME.recovery.zip.md5 $SMB_FOLDER_KERNEL\\$NEBULA_VERSION\\$NEBULA_FILENAME.recovery.zip.md5"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/checksum $SMB_FOLDER_KERNEL\\$NEBULA_VERSION\\checksum"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/cm-kernel.zip $SMB_FOLDER_KERNEL\\$NEBULA_VERSION\\cm-kernel.zip"
		return
	fi
	...
	if [ ! -z "$SSH_FTP_REMOTE" ]; then
		...
		mkdir -p ~/bbuild_transfer/$NEBULA_VERSION

		cp $REPACK_PATH/$NEBULA_FILENAME.recovery.zip ~/bbuild_transfer/$NEBULA_VERSION
		cp $REPACK_PATH/$NEBULA_FILENAME.recovery.zip.md5 ~/bbuild_transfer/$NEBULA_VERSION
		cp $REPACK_PATH/checksum ~/bbuild_transfer/$NEBULA_VERSION
		cp $REPACK_PATH/cm-kernel.zip ~/bbuild_transfer/$NEBULA_VERSION
...
step9_send_finished_mail()
{
	...
	if [ -z "$FINISH_MAIL_TO" ]; then
		...
		cat $ROOT_PATH/compile.log | /usr/bin/mailx -s "Compilation for $KERNEL_NAME $NEBULA_VERSION finished!!!" $FINISH_MAIL_TO
	fi
}
...
display_help()
{
	...
	echo "  Kernel Filename: $NEBULA_FILENAME"
	echo
	echo "======================================================================"
}
