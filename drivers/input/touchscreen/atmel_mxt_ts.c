...
{
	const struct mxt_platform_data *pdata = data->pdata;
	struct mxt_config_info *config = NULL;
	int i;
	...
	for (i = 0; i < pdata->config_array_size; i++) {
		config = &pdata->config_array[i];
		if (data->info.family_id == config->family_id &&
			data->info.variant_id == config->variant_id &&
			data->info.version == config->version &&
			data->info.build == config->build &&
			data->rev_id == config->rev_id) {

			/* Check vendor ids */
			if (memcmp(data->vendor_ids, config->vendor_ids,
					pdata->vendor_id_num) == 0)
				break;
		}
	}
...
start:
	...
	data->vendor_ids = devm_kzalloc(dev,
			data->pdata->vendor_id_num * sizeof(u8),
			GFP_KERNEL);
	if (!data->vendor_ids)
		return -ENOMEM;

	for (i = 0; i < data->pdata->vendor_id_num; i++) {
		data->vendor_ids[i] =
			data->lockdown_info[data->pdata->vendor_id_bytes[i]];
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	ssize_t count = sprintf(buf,
			"family_id=0x%02x, variant_id=0x%02x, version=0x%02x, build=0x%02x\n",
...
static void cover_mode_set(struct mxt_data *data, int enable)
{
	...
	if (enable) {
		...
		error = mxt_write_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YSIZE, data->pdata->config_array[index].ysize);
		if (error) {
			dev_err(&data->client->dev, "write to t100 ysize failed!\n");
			return;
		}

		error = mxt_write_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YRANGE_LSB, data->pdata->config_array[index].yrange_lsb);
		if (error) {
			dev_err(&data->client->dev, "write to t100 yrange lsb failed!\n");
			return;
		}

		error = mxt_write_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YRANGE_MSB, data->pdata->config_array[index].yrange_msb);
		if (error) {
			dev_err(&data->client->dev, "write to t100 yrange msb failed!\n");
			return;
		}

		error = mxt_write_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_TCHTHR, data->pdata->config_array[index].tchthr);
		if (error) {
		...
		error = mxt_write_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_INTTHR, data->pdata->config_array[index].intthr);
		if (error) {
		...
		error = mxt_write_object(data, MXT_SPT_AUXTOUCHCONFIG_T104, MXT_AUXTCHCFG_INTTHRHYSTX, data->pdata->config_array[index].intthrhystx);
		if (error) {
			dev_err(&data->client->dev, "write to t104 intthrhystx failed!\n");
			return;
		}
		...
		error = mxt_write_object(data, MXT_SPT_AUXTOUCHCONFIG_T104, MXT_AUXTCHCFG_INTTHRHYSTY, data->pdata->config_array[index].intthrhysty);
		if (error) {
			dev_err(&data->client->dev, "write to t104 intthrhysty failed!\n");
			return;
		}
		...
		error = mxt_write_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YSIZE, data->restore_reg.ysize_nor);
		if (error) {
			dev_err(&data->client->dev, "write to t100 ysize failed!\n");
			return;
		}

		error = mxt_write_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YRANGE_LSB, data->restore_reg.yrange_lsb_nor);
		if (error) {
			dev_err(&data->client->dev, "write to t100 yrange lsb failed!\n");
			return;
		}

		error = mxt_write_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YRANGE_MSB, data->restore_reg.yrange_msb_nor);
		if (error) {
			dev_err(&data->client->dev, "write to t100 yrange msb failed!\n");
			return;
		}
		...
		error = mxt_write_object(data, MXT_SPT_AUXTOUCHCONFIG_T104, MXT_AUXTCHCFG_INTTHRHYSTX, data->restore_reg.intthrhystx_nor);
		if (error) {
			dev_err(&data->client->dev, "write to t104 intthrhystx failed!\n");
			return;
		}
		...
		error = mxt_write_object(data, MXT_SPT_AUXTOUCHCONFIG_T104, MXT_AUXTCHCFG_INTTHRHYSTY, data->restore_reg.intthrhysty_nor);
		if (error) {
			dev_err(&data->client->dev, "write to t104 intthrhysty failed!\n");
			return;
		}
...
static void mxt_dump_value(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	for (j = 0; j < pdata->vendor_id_num; j++) {
		len += snprintf(tmp + len, 256 - len, "%d ",
			pdata->vendor_id_bytes[j]);
	}
	dev_info(dev, "vendor id bytes = %s\n", tmp);
	...
	for (i = 0; i < pdata->config_array_size; i++) {
		...
		for (j = 0; j < pdata->vendor_id_num; j++) {
			len += snprintf(tmp + len, 256 - len, "0x%x ",
				pdata->config_array[i].vendor_ids[j]);
		}
		dev_info(dev, "config[%d]: vendor ids = %s\n", i, tmp);
	}
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	prop = of_find_property(np, "atmel,vendor-id-byte", NULL);
	if (prop && prop->length) {
		pdata->vendor_id_bytes = devm_kzalloc(dev,
				prop->length,
				GFP_KERNEL);
		if (!pdata->vendor_id_bytes)
			return -ENOMEM;
		pdata->vendor_id_num = prop->length / sizeof(u8);
		ret = of_property_read_u8_array(np, "atmel,vendor-id-byte",
				pdata->vendor_id_bytes,
				pdata->vendor_id_num);
		if (ret < 0) {
			dev_err(dev, "Error reading vendor id bytes\n");
			return ret;
		}
	} else {
		dev_err(dev, "Unable to get vendor id bytes\n");
		return -EINVAL;
	}
	...
	for_each_child_of_node(np, temp) {
		...
		prop = of_find_property(temp, "atmel,vendor-id", NULL);
		if (prop && prop->length) {
			if (pdata->vendor_id_num != prop->length / sizeof(u8)) {
				dev_err(dev, "Invalid vendor id length\n");
				return -EINVAL;
			}
			info->vendor_ids = devm_kzalloc(dev,
					prop->length,
					GFP_KERNEL);
			if (!info->vendor_ids)
				return -ENOMEM;
			ret = of_property_read_u8_array(temp,
					"atmel,vendor-id",
					info->vendor_ids,
					pdata->vendor_id_num);
			if (ret) {
				dev_err(dev, "Error reading vendor ids\n");
				return ret;
		...
		ret = of_property_read_u32(temp, "atmel,ysize", &temp_val);
		if (ret) {
			dev_err(dev, "Unable to read t100 ysize\n");
			return ret;
		} else
			info->ysize = temp_val;

		ret = of_property_read_u32(temp, "atmel,yrange-lsb", &temp_val);
		if (ret) {
			dev_err(dev, "Unable to read t100 yrange lsb\n");
			return ret;
		} else
			info->yrange_lsb = temp_val;

		ret = of_property_read_u32(temp, "atmel,yrange-msb", &temp_val);
		if (ret) {
			dev_err(dev, "Unable to read t100 yrange msb\n");
			return ret;
		} else
			info->yrange_msb = temp_val;

		ret = of_property_read_u32(temp, "atmel,tchthr", &temp_val);
		if (ret) {
			dev_err(dev, "Unable to read t100 tchthr\n");
			return ret;
		} else
			info->tchthr = temp_val;
		...
		ret = of_property_read_u32(temp, "atmel,intthr", &temp_val);
		if (ret) {
			dev_err(dev, "Unable to read t104 intthr\n");
			return ret;
		} else
			info->intthr = temp_val;

		ret = of_property_read_u32(temp, "atmel,intthrhyst", &temp_val);
		if (ret) {
			dev_err(dev, "Unable to read t104 intthrhyst\n");
			return ret;
		} else
		...
		ret = of_property_read_u32(temp, "atmel,intthrhystx", &temp_val);
		if (ret) {
			dev_err(dev, "Unable to read t104 intthrhystx\n");
			return ret;
		} else
			info->intthrhystx = temp_val;
		...
		ret = of_property_read_u32(temp, "atmel,intthrhysty", &temp_val);
		if (ret) {
			dev_err(dev, "Unable to read t104 intthrhysty\n");
			return ret;
		} else
			info->intthrhysty = temp_val;
...
static void normal_mode_reg_save(struct mxt_data *data)
{
	...
	error = mxt_read_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YSIZE, &reg_val);
	if (error) {
		dev_err(&data->client->dev, "write to t100 ysize failed!\n");
		return;
	} else
		data->restore_reg.ysize_nor = reg_val;

	error = mxt_read_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YRANGE_LSB, &reg_val);
	if (error) {
		dev_err(&data->client->dev, "write to t100 yrange lsb failed!\n");
		return;
	} else
		data->restore_reg.yrange_lsb_nor = reg_val;

	error = mxt_read_object(data, MXT_TOUCH_MULTI_T100, MXT_MULTITOUCH_YRANGE_MSB, &reg_val);
	if (error) {
		dev_err(&data->client->dev, "write to t100 yrange msb failed!\n");
		return;
	} else
		data->restore_reg.yrange_msb_nor = reg_val;
	...
	error = mxt_read_object(data, MXT_SPT_AUXTOUCHCONFIG_T104, MXT_AUXTCHCFG_INTTHRHYSTX, &reg_val);
	if (error) {
		dev_err(&data->client->dev, "write to t104 intthrhystx failed!\n");
		return;
	} else
		data->restore_reg.intthrhystx_nor = reg_val;
	...
	error = mxt_read_object(data, MXT_SPT_AUXTOUCHCONFIG_T104, MXT_AUXTCHCFG_INTTHRHYSTY, &reg_val);
	if (error) {
		dev_err(&data->client->dev, "write to t104 intthry failed!\n");
		return;
	} else
		data->restore_reg.intthrhysty_nor = reg_val;
}
